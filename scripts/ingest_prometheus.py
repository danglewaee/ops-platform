...
from ops_platform.prometheus_ingestion import load_prometheus_bundle
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Query Prometheus, normalize the results, and ingest them into SQLite.")
    parser.add_argument("--config", required=True, help="Path to a Prometheus TOML or JSON query config.")
    parser.add_argument("--stream-id", required=True, help="Stable stream identifier for persisted telemetry.")
    parser.add_argument("--start", help="Range start as ISO-8601 or unix epoch seconds.")
    parser.add_argument("--end", help="Range end as ISO-8601 or unix epoch seconds.")
    parser.add_argument("--lookback-minutes", type=int, help="Pull a relative window ending now or at --end.")
    ...
    parser.add_argument("--source", default="prometheus", help="Source label stored with the ingested stream.")
    ...
    parser.add_argument(
        "--description",
        default="Prometheus telemetry replay evaluated in shadow mode.",
