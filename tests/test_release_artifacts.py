...
class ReleaseArtifactTests(unittest.TestCase):
    def test_build_release_artifacts_writes_manifest_and_outputs(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(manifest["v2_testbed_benchmark"]["summary"]["case_count"], 5)
            self.assertEqual(manifest["v2_testbed_benchmark"]["summary"]["action_match_rate_pct"], 100.0)
        ...
        self.assertEqual(saved_manifest["v2_testbed_benchmark"]["summary"]["top2_root_cause_accuracy_pct"], 100.0)
        self.assertIn("# Release Overview", overview)
        self.assertIn("Resume and website copy", overview)
        self.assertIn("V2 boutique-like benchmark", overview)
